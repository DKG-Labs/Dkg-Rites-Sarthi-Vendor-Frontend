...
const SyncPOModal = ({ isOpen, onClose, onSuccess, vendorCode: propVendorCode, plantId }) => {
    ...
    const handleSync = async (e) => {
        ...
        try {
            ...
            if (syncType === 'IBS_CASE_NO') {
                ...
                const caseNo = data?.CASE_NO || data?.caseNo || res?.CASE_NO;

                if (res && (res.resultFlag === 1 || res.status === 1) && caseNo && String(caseNo).trim() !== '') {
                    setIbsResult(data);
                    setView('review');
                    setStatus('idle');
                } else {
                    const serverMsg = res?.message || data?.message;
                    const friendlyMsg = serverMsg 
                        ? `IBS Info: ${serverMsg}` 
                        : `No IBS Case Number was found for PO ${ibsPayload.PO_NO}. Please ensure this Purchase Order is registered in RITES IBS portal.`;
    ...
    const renderReviewView = () => {
            ...
            return (
                <div style={styles.body}>
                    ...
                    <div style={styles.reviewGridCard}>
                        ...
                        {ibsResult.UPDATED_DT && (
                            <div style={{ ...styles.reviewGridItem, gridColumn: 'span 2' }}>
                                <span style={styles.reviewGridLabel}>Updated Date</span>
                                <span style={styles.reviewGridValue}>{ibsResult.UPDATED_DT}</span>
                            </div>
                        )}
                    </div>

                    <div style={styles.successBanner}>
                        ✨ <strong>Case Number Found:</strong> IBS Case Number retrieved successfully from RITES IBS portal.
                    </div>
        ...
        if (fetchedData) {
            h = fetchedData.PoHdr || fetchedData.data?.PoHdr || fetchedData.data?.MMP_PO_HDR || fetchedData.MMP_PO_HDR || fetchedData.data?.MMP_POMA_HDR || fetchedData.MMP_POMA_HDR || {};
            d = fetchedData.PoDtl || fetchedData.data?.PoDtl || fetchedData.data?.MMP_PO_DTL || fetchedData.MMP_PO_DTL || fetchedData.data?.MMP_POMA_DTL || fetchedData.MMP_POMA_DTL || [];
        }
        ...
        return (
            <div style={styles.body}>
                ...
                {isMismatch ? (
                    ...
                    <div style={{ ...styles.formGroup, marginBottom: '18px' }}>
