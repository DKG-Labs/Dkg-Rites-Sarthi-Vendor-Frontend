...
const CompletedCallsDashboard = ({ vendorCode, plantId }) => {
    ...
    const handleViewDetails = async (callId) => {
        try {
            ...
            if (detailedCall && detailedCall.callType === 'PROCESS' && detailedCall.callNo) {
                try {
                    const pDetails = await inspectionCallService.getProcessCallDetails(detailedCall.callNo);
...
const CallDetailsView = ({ call, transitionHistory, loadingHistory, onBack, processCallDetails, loadingDetails }) => {
    return (
        <div className="fade-in">
            <button 
                onClick={onBack}
            ...
            <div style={{ background: 'linear-gradient(135deg, #0d3b3f 0%, #21808d 100%)', borderRadius: '24px', padding: '32px', color: '#fff', marginBottom: '24px', boxShadow: '0 10px 25px -5px rgba(33,128,141,0.3)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div>
                        ...
                        <h2 style={{ fontSize: '32px', fontWeight: 900, margin: 0, letterSpacing: '-0.02em', color: '#ffffff' }}>{call.callNo}</h2>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 350px', gap: '24px', alignItems: 'start' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                    {/* Dynamic Breakdown based on Call Type */}
                    {call.callType === 'PROCESS' ? (
                        <div style={{ background: '#fff', borderRadius: '24px', border: '1px solid #e2e8f0', padding: '24px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px' }}>
                                ...
                                <h3 style={{ margin: 0, fontSize: '18px', fontWeight: 800, color: '#1e293b' }}>Process Call Details</h3>
                            </div>

                            {processCallDetails ? (
                                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '16px' }}>
                                    <div style={{ background: '#f8fafc', padding: '16px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
                                        <div style={{ fontSize: '11px', fontWeight: 700, color: '#64748b', textTransform: 'uppercase', marginBottom: '4px' }}>Drawing No</div>
                                        <div style={{ fontWeight: 800, color: '#0f172a', fontSize: '15px' }}>{processCallDetails.drawingNo || '-'}</div>
                                    </div>
                                    <div style={{ background: '#f8fafc', padding: '16px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
                                        <div style={{ fontSize: '11px', fontWeight: 700, color: '#64748b', textTransform: 'uppercase', marginBottom: '4px' }}>UOM</div>
                                        <div style={{ fontWeight: 800, color: '#0f172a', fontSize: '15px' }}>{processCallDetails.uom || '-'}</div>
                                    </div>
                                    <div style={{ background: '#f8fafc', padding: '16px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
                                        <div style={{ fontSize: '11px', fontWeight: 700, color: '#64748b', textTransform: 'uppercase', marginBottom: '4px' }}>Qty On Order</div>
                                        <div style={{ fontWeight: 800, color: '#0f172a', fontSize: '15px' }}>{processCallDetails.qtyOnOrder?.toLocaleString() || '-'}</div>
                                    </div>
                                    <div style={{ background: '#f8fafc', padding: '16px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
                                        <div style={{ fontSize: '11px', fontWeight: 700, color: '#64748b', textTransform: 'uppercase', marginBottom: '4px' }}>Qty Accepted Till Now</div>
                                        <div style={{ fontWeight: 800, color: '#0f172a', fontSize: '15px' }}>{processCallDetails.qtyAcceptedTillNow?.toLocaleString() || '-'}</div>
                                    </div>
                                    <div style={{ background: '#f8fafc', padding: '16px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
                                        <div style={{ fontSize: '11px', fontWeight: 700, color: '#64748b', textTransform: 'uppercase', marginBottom: '4px' }}>Qty Desired For Final</div>
                                        <div style={{ fontWeight: 800, color: '#2563eb', fontSize: '15px' }}>{processCallDetails.qtyDesiredForFinal?.toLocaleString() || '-'}</div>
                                    </div>
                                    <div style={{ background: '#f8fafc', padding: '16px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
                                        <div style={{ fontSize: '11px', fontWeight: 700, color: '#64748b', textTransform: 'uppercase', marginBottom: '4px' }}>Qty Due</div>
                                        <div style={{ fontWeight: 800, color: '#0f172a', fontSize: '15px' }}>{processCallDetails.qtyDue?.toLocaleString() || '-'}</div>
                                    </div>
                                    <div style={{ background: '#f8fafc', padding: '16px', borderRadius: '12px', border: '1px solid #e2e8f0', gridColumn: '1 / -1' }}>
                                        <div style={{ fontSize: '11px', fontWeight: 700, color: '#64748b', textTransform: 'uppercase', marginBottom: '4px' }}>Production Initiation Date</div>
                                        <div style={{ fontWeight: 800, color: '#0f172a', fontSize: '15px' }}>{formatDateDDMMYY(processCallDetails.productionInitiationDate) || '-'}</div>
                                    </div>
                                </div>
                            ) : (
                                <div style={{ color: '#64748b', fontSize: '14px', fontStyle: 'italic', padding: '20px', background: '#f8fafc', borderRadius: '12px' }}>
                                    {loadingDetails ? 'Loading details...' : 'Process details not available.'}
                                </div>
                            )}
                        </div>
                    ) : (
                        <div style={{ background: '#fff', borderRadius: '24px', border: '1px solid #e2e8f0', padding: '24px' }}>
                            ...
                            {(call.lots || []).length > 0 ? (call.lots || []).map((lot, lIdx) => (
                                ...
                                <div style={{ color: '#64748b', fontSize: '14px', fontStyle: 'italic', padding: '20px', background: '#f8fafc', borderRadius: '12px' }}>
                                    No lots available for this call.
                                </div>
                            )}
                        </div>
                    )}
