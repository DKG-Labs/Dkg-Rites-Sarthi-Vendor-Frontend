...
const SyncPOModal = ({ isOpen, onClose, onSuccess, vendorCode: propVendorCode, plantId }) => {
    ...
    const fetchVendorPos = async (vcode) => {
        ...
        setPoLoading(true);
        try {
            const data = await immsService.getPoAssigned(cleanVcode, 'Elastic Rail Clips');
    ...
    const handleSync = async (e) => {
        ...
        try {
            ...
            if (syncType === 'IBS_CASE_NO') {
                ...
                const res = await immsService.getIbsCaseNo(ibsPayload);
            ...
            let immsToken = localStorage.getItem('imms_token') || sessionStorage.getItem('imms_token');
            if (!immsToken && immsService.authenticateIMMS) {
                immsToken = await immsService.authenticateIMMS();
            }
            ...
            if (syncType === 'POMA DATA') {
                ...
                if (formData.poNo) {
                    try {
                        const poDateRes = await immsService.getPoDateByPoNo(formData.poNo.trim());
                ...
                console.log('Final MA Payload being sent:', maPayload);
                const res = await (immsService.getIMMSMAData ? immsService.getIMMSMAData(maPayload) : immsService.getIMMSPOData(maPayload));
                ...
                const res = await immsService.getIMMSPOData(payload);
                const hasData = res && (
                    res.PoHdr || 
                    res.data?.PoHdr || 
                    res.data?.MMP_PO_HDR || 
    ...
    const renderReviewView = () => {
        ...
        if (fetchedData) {
            h = fetchedData.PoHdr || fetchedData.data?.MMP_PO_HDR || fetchedData.MMP_PO_HDR || fetchedData.data?.MMP_POMA_HDR || fetchedData.MMP_POMA_HDR || fetchedData.data?.PoHdr || {};
            d = fetchedData.PoDtl || fetchedData.data?.MMP_PO_DTL || fetchedData.MMP_PO_DTL || fetchedData.data?.MMP_POMA_DTL || fetchedData.MMP_POMA_DTL || fetchedData.data?.PoDtl || [];
        }

        const allowedCategory = "Elastic Rail Clips";
        const dashboardRole = "Elastic Rail Clip";
