...
const EMPTY = {
    dateOfReceipt: '', particleSize: '', sourceOfSupply: '',
    invoiceNumber: '', ewayBillNumber: '', invoiceDate: '',
    ewayBillDate: '', tcNumber: '', tcDate: '',
    quantity: '', document: null,
};

const Silica = ({ entries, setEntries, approvedSuppliers, sourceObjects, allInvoices }) => {
    ...
    const availableSuppliers = React.useMemo(() => {
        if (sourceObjects && sourceObjects.length > 0) {
            const filtered = sourceObjects.filter(src => src.materialName?.trim().toLowerCase() === 'silica');
    ...
    const validate = () => {
        ...
        const ps = parseFloat(form.particleSize);
        if (!form.particleSize || isNaN(ps)) errs.particleSize = 'Required';
        else if (ps >= 20) errs.particleSize = '❌ Particle size must be less than 20 microns. Entry cannot be saved.';
    ...
    const psVal = parseFloat(form.particleSize);
    const psSizeError = form.particleSize && !isNaN(psVal) && psVal >= 20;

    return (
        <div className="fade-in">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                <div>
                    <h2 style={{ fontSize: 'var(--fs-xl)', fontWeight: '800', color: 'var(--text-main)', margin: '0 0 4px 0' }}>Silica</h2>
                    <p style={{ color: 'var(--text-muted)', fontSize: '12px', margin: 0 }}>Particle size validated – must be less than 20 microns</p>
                </div>
                {view === 'list' && (
            ...
            {view === 'list' ? (
                <InventoryTable
                    ...
                    columns={[
                        ...
                        { key: 'particleSize', label: 'Particle Size (µm)' },
                ...
                <div className="inv-form-container">
                    <div className="inv-form-header">
                        <h3>{editId ? 'Edit Entry' : 'Add Silica Entry'}</h3>
                    ...
                    <form onSubmit={handleSubmit}>
                        <div className="inv-form-grid">
                            ...
                            {/* Particle Size with live validation */}
                            <div className="form-group">
                                <label className="form-label">Particle Size (microns) <span style={{ color: '#dc2626' }}>*</span></label>
                                <input
                                    type="number" step="0.01" min="0"
                                    className={`form-input${psSizeError ? ' input-error' : ''}`}
                                    value={form.particleSize}
                                    onChange={e => { set('particleSize', e.target.value); setErrors(prev => ({ ...prev, particleSize: '' })); }}
                                ...
                                {psSizeError && (
                                    <div style={{
                                        ...
                                        ❌ Particle size must be strictly less than 20 microns. This entry CANNOT be saved.
                                    </div>
                                )}
                                {errors.particleSize && !psSizeError && <span className="error-msg">{errors.particleSize}</span>}
                            </div>
                        ...
                        <div style={{ marginTop: '28px', display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
                            ...
                            <button type="submit" className="btn-primary" style={{ padding: '0.7rem 2.5rem' }} disabled={psSizeError}>
